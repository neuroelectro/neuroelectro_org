...
def write_validation_spreadsheet():
    ...
    with open('validation_spreadsheet_robot.csv','wb') as fout:
        ...
        articles = m.Article.objects.filter(Q(datatable__datasource__neuronconceptmap__times_validated__gte = 1)).distinct()
        ...
        for j,a in enumerate(selected_articles):
            ...
            for k,dt in enumerate(dts):
                ...
                ncms = m.NeuronConceptMap.objects.filter(Q(times_validated__gte = 1) & 
                ...
                ecms = m.EphysConceptMap.objects.filter(Q(times_validated__gte = 1) & 
                ...
                nedms = m.NeuronEphysDataMap.objects.filter(neuron_concept_map__in = ncms, 
