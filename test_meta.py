...
def process_articles():
    ...
    while current < n_articles:
        call = 'python process_article.py' + str(articles[get_next_article_index()].id)
...
#         pipe.add_article_full_text_from_file(f, directory)
    ...
    while i < threadNumber:
        #process_article(articles[get_next_article_index()])
        t = Thread(target = process_articles)
        i += 1
        t.start()
